...
def test_inventory_item_date_added():
    """Test InventoryItem date_added initialization"""
    # Test with default date
    item1 = InventoryItem("Test Item", 10, 9.99)
    assert isinstance(item1.date_added, str)
    assert len(item1.date_added) > 0

    # Test with custom date
    custom_date = "2024-01-01 12:00:00"
    item2 = InventoryItem("Test Item", 10, 9.99, custom_date)
    assert item2.date_added == custom_date


def test_ensure_file_exists(tmp_path):
    """Test file creation and existence check"""
    # Test creating new file
    file_path = tmp_path / "new_inventory.csv"
    manager = InventoryManager(filename=str(file_path))
    assert file_path.exists()

    # Test existing file
    manager2 = InventoryManager(filename=str(file_path))
    assert file_path.exists()


def test_save_and_load_inventory(inventory_manager):
    ...
    new_manager = InventoryManager(filename=inventory_manager.filename)

    # Verify loaded items
    assert len(new_manager.items) == 2
    assert new_manager.items[0].name == "Item 1"
    assert new_manager.items[0].quantity == 10
    assert new_manager.items[0].price == 9.99
    assert new_manager.items[1].name == "Item 2"
    assert new_manager.items[1].quantity == 5
    assert new_manager.items[1].price == 4.99


def test_inventory_item_edge_cases():
    """Test edge cases for InventoryItem"""
    # Test with zero quantity
    item1 = InventoryItem("Zero Item", 0, 9.99)
    assert item1.quantity == 0

    # Test with zero price
    item2 = InventoryItem("Free Item", 10, 0.0)
    assert item2.price == 0.0

    # Test with very large numbers
    item3 = InventoryItem("Large Item", 1000000, 999999.99)
    assert item3.quantity == 1000000
    assert item3.price == 999999.99

    # Test with empty name
    item4 = InventoryItem("", 10, 9.99)
    assert item4.name == ""


def test_inventory_manager_edge_cases(inventory_manager):
    """Test edge cases for InventoryManager"""
    # Test adding item with empty name
    assert inventory_manager.add_item("", 10, 9.99)

    # Test adding item with zero quantity
    assert inventory_manager.add_item("Zero Item", 0, 9.99)

    # Test adding item with zero price
    assert inventory_manager.add_item("Free Item", 10, 0.0)

    # Test removing empty string
    assert not inventory_manager.remove_item("")

    # Test searching with empty string
    assert inventory_manager.search_item("") is None
