...
class InventoryItem:
    def __init__(self, name: str, quantity: int, price: float, date_added: str = None):
        ...
        self.price = price
        self.date_added = date_added or datetime.today().strftime("%Y-%m-%d %H:%M:%S")
...
class InventoryManager:
    ...
    def sort_by_name(self):
        quicksort(self.items, 0, len(self.items) - 1, lambda x: x.name)
        self._save_inventory()

    def sort_by_quantity(self):
        quicksort(self.items, 0, len(self.items) - 1, lambda x: x.quantity)
        self._save_inventory()

    def sort_by_price(self):
        quicksort(self.items, 0, len(self.items) - 1, lambda x: x.price)
        self._save_inventory()

    def sort_by_date_added(self):
        quicksort(self.items, 0, len(self.items) - 1, lambda x: x.date_added)
        self._save_inventory()
